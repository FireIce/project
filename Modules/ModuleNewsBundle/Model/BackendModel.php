            ...
            if ($acl->checkUserPermissions($request->get('id'), $service_module, false, $acl->getValueMask('provemaineditor'))) {
                ...
                foreach ($plugins as $plugin) {
                    ...
                    if (count($result) > 0) {
                        ...
                        if ($result['final'] == 'W') $query = $this->em->createQuery("UPDATE ".$this->bundle_name.':'.$this->entity_name." md SET md.final='N', md.eid = ".$hid." WHERE md.idd = ".$result['idd']." AND (md.final = 'Y' OR md.final = 'W') AND md.eid IS NULL AND md.row_id = ".$request->get('id_row'));
                        elseif ($result['final'] == 'Y') $query = $this->em->createQuery("UPDATE ".$this->bundle_name.':'.$this->entity_name." md SET md.eid = ".$hid." WHERE md.idd = ".$result['idd']." AND (md.final = 'Y' OR md.final = 'W') AND md.eid IS NULL AND md.row_id = ".$request->get('id_row'));
                        $query->getResult();
